...
router.get("/list", (req, res, next) => {
    ...
    // fetch today's games
    let apiString = `${constants_1.mlbApi.url}/api/v1/schedule/?sportId=1&date=${today_date}`;
    axios_1.default.get(apiString)
        ...
        return Promise.all(response.data.dates[0].games.map(game => {
            return axios_1.default.get(`${constants_1.mlbApi.url}${game.link}`);
        }));
    })
        ...
        return Promise.all([
            ...
            Promise.all(playersToQuery.map(playerLink => {
                return axios_1.default.get(`${constants_1.mlbApi.url}${playerLink}`);
            }))
        ]);
    })
            ...
            const gameObj = {
                ...
                "state": {
                    "status": {
                        ...
                        "start_time": Object.assign({ "display": `${game.gameData.datetime.time} ${game.gameData.datetime.ampm}` }, game.gameData.datetime)
                    }
                },
